...
build-%:
	...
	docker build --target neptuneplus-simple-dependencies-$* -t $(IMAGE_BASE_NAME):$* .
	@echo "Successfully built $(IMAGE_BASE_NAME):$*"

clean:
	@echo "Removing Docker images..."
	@for func in $(FUNCTIONS); do \
		if docker images -q $(IMAGE_BASE_NAME):$$func &> /dev/null; then \
			docker rmi $(IMAGE_BASE_NAME):$$func; \
			echo "Removed $(IMAGE_BASE_NAME):$$func"; \
		else \
			echo "Image $(IMAGE_BASE_NAME):$$func does not exist."; \
		fi \
	done
...
push-%:
	...
	docker image push $(IMAGE_BASE_NAME):$*
	@echo "Successfully pushed $(IMAGE_BASE_NAME):$*"
